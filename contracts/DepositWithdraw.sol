...
contract DepositWithdraw is Claimable, Withdrawable, TokenDestructible {
   ...
   */
  function setWithdrawWallet(address payable _wallet) public onlyOwner returns (bool) {
    require(_wallet != address(0));
    wallet = _wallet;
   ...
   */
  function receiveApproval(address _from, uint256 _value, address _token, bytes memory _extraData) public onlyOwner {
   ...
   */
  function recordWithdraw(uint256 _time, address payable _to, uint256 _value) public onlyOwner {
   ...
   */
  function withdrawTokenToDefault(
    address _token, 
    address _params, 
    uint256 _time, 
    uint256 _value, 
