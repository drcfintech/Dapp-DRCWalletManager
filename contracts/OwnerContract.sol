...
import '../solidity-lib/openzeppelin-solidity-112/contracts/ownership/DelayedClaimable.sol';
...
contract OwnerContract is DelayedClaimable {
   ...
   */
  function bindContract(address _contract) public onlyOwner returns(bool) {
   ...
   */
  function changeOwnershipto(address _nextOwner) public onlyOwner {
   ...
   *
   */
  function ownedOwnershipTransferred() public onlyOwner returns(bool) {
    require(address(ownedContract) != address(0));
    if (ownedContract.owner() == pendingOwnedOwner) {
      // the pending owner has already taken the ownership  
      ownedContract = Claimable(address(0));
